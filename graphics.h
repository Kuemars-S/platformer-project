...
void draw_parallax_background() {
    ...
    float initial_offset      = -(PlayerManager::getInstance().get_player().get_pos().x * PARALLAX_PLAYER_SCROLLING_SPEED + game_frame * PARALLAX_IDLE_SCROLLING_SPEED);
...
void draw_game_overlay() {
    ...
    // Score
    Vector2 score_dimensions = MeasureTextEx(menu_font, std::to_string(PlayerManager::getInstance().get_total_player_score()).c_str(), ICON_SIZE, 2.0f);
    Vector2 score_position = {GetRenderWidth() - score_dimensions.x - ICON_SIZE, slight_vertical_offset};
    DrawTextEx(menu_font, std::to_string(PlayerManager::getInstance().get_total_player_score()).c_str(), score_position, ICON_SIZE, 2.0f, WHITE);
    ...
    for (size_t row = 0; row < Level::getInstance().get_current_level().rows; ++row) {
        for (size_t column = 0; column < Level::getInstance().get_current_level().columns; ++column) {
            ...
            Vector2 pos = {
                    ...
                    (static_cast<float>(column) - PlayerManager::getInstance().get_player().get_pos().x) * cell_size + horizontal_shift,
                    static_cast<float>(row) * cell_size
            };

            // Draw the level itself
            char cell = Level::getInstance().get_level_cell(row, column);
            switch (cell) {
                case WALL:
...
void draw_player() {
    ...
    Vector2 pos = {
            horizontal_shift,
            PlayerManager::getInstance().get_player().get_pos().y * cell_size
    };
...
void draw_enemies() {
    ...
    for (auto &enemy : EnemiesManager::getInstance().get_enemies()) {
        ...
        Vector2 pos = {
                (enemy.get_pos().x - PlayerManager::getInstance().get_player().get_pos().x) * cell_size + horizontal_shift,
